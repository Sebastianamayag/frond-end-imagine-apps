...
export const FormShippingsView = ({ navigation, route }) => {
    ...
    const [date, setDate] = useState(data ? new Date(data.date) : new Date());
    ...
    const handleSumbit = () => {
        ...
        if (!data) {
            dispatch(createShippings(datas,navigation));
        } else {
            console.log(datas);
            dispatch(updateShippings({...datas,id:data.id},navigation));
        }
    }
